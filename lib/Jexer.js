...
Jexer.prototype.tokenize = function (input) {
    let line = 1, column = 1;
    const tokens = [];
    ...
    for ( let i = 0 ; i < chars.length ; i++ ) {
        ...
        if ( matched_rules.length === 0 ) {
            if ( prev_matched_rules.length === 0 ) {
                ...
                let type = prev_matched_rules[0].token;
                if ( typeof type === 'function' ) {
                    type = type(prev_yytext, token_line, token_column);
                }
                console.log('token: '+type+' '+JSON.stringify(prev_yytext));
                if (type) {
                    tokens.push({
                        type: type,
                        text: prev_yytext
    ...
    if ( matched_rules.length === 0 ) {
        ...
        let type = matched_rules[0].token;
        if (typeof type === 'function') {
